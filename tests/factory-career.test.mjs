...
test('legacy version-one saves migrate without losing progress, machines or coins', () => {
  const old = new FactoryGame({ shop: false }); run(old, 15);
  const save = JSON.parse(old.serialize()); delete save.career;
  const restored = new FactoryGame({ shop: false }); assert.equal(restored.restore(JSON.stringify(save)), true);
...
test('offers only request unlocked goods and remain bounded for long-running factories', () => {
  ...
  const game = new FactoryGame({ shop: false });
  for (let tier = 0; tier < 3; tier++) for (const round of [0, 3, 50, 99999]) {
    ...
    for (const offer of game.offers) {
      ...
      assert.ok(offer.duration >= 120 && offer.duration <= 220); assert.ok(offer.points >= 1 && offer.points <= 3);
    }
  }
});
test('contracts count only new sales while preserving ordinary income and main-order credit', () => {
  const game = new FactoryGame({ shop: false }); sell(game, 'bread', 10); const wallet = game.state.coins;
...
test('claiming a completed contract rewards once, including after save and reload', () => {
  const game = new FactoryGame({ shop: false }); game.acceptContract(0); sell(game, 'bread', 6);
  const ready = new FactoryGame({ shop: false }); assert.equal(ready.restore(game.serialize()), true);
  ...
  const saved = new FactoryGame({ shop: false }); assert.equal(saved.restore(ready.serialize()), true); assert.equal(saved.claimContract().ok, false);
...
test('deadline accepts the last tick, and a ready reward never expires', () => {
  ...
  sell(game, 'bread', 5); game.state.time = 119.9;
  const belt = game.place('belt', 1, 1).building; game.place('depot', 2, 1); belt.output = 'bread';
  game.update(.1); assert.equal(game.state.time, 120); assert.equal(game.contract.status, 'ready');
...
test('expired and abandoned contracts cost no coins and cannot yield rewards', () => {
  ...
  run(game, 120); assert.equal(game.contract.status, 'expired'); assert.equal(game.state.coins, coins);
...
test('pause and reload preserve contract time; double speed advances production and deadline together', () => {
  ...
  run(game, 100); assert.equal(game.state.time, 0); assert.equal(game.contract.deadline, 120);
  const restored = new FactoryGame({ shop: false }); assert.equal(restored.restore(game.serialize()), true); assert.equal(restored.contract.status, 'active');
  restored.state.paused = false; restored.state.speed = 2; run(restored, 60); assert.equal(restored.state.time, 120); assert.equal(restored.contract.status, 'expired');
});
...
test('production research preserves completion, original ingredients and valid paused saves', () => {
  const game = new FactoryGame({ shop: false }), machine = game.at(5, 2);
...
test('packaging yields integer coins for every saleable item and round-trips correctly', () => {
  for (let level = 1; level <= 3; level++) {
    const game = new FactoryGame({ shop: false }); game.state.career.research.value = level;
...
test('corrupted career saves cannot replace a working factory or inject rewards', () => {
  const game = new FactoryGame({ shop: false }); game.acceptContract(0); const before = game.serialize();
...
test('starter can fulfill the easy challenge while difficult contracts reward improved capacity', () => {
  const starter = new FactoryGame({ shop: false }); starter.acceptContract(0); run(starter, 60); assert.equal(starter.contract.status, 'ready');
  const difficult = new FactoryGame({ shop: false }); difficult.acceptContract(1); run(difficult, 60); assert.equal(difficult.contract.status, 'active'); assert.ok(difficult.contract.progress.bread < 18);
  const improved = new FactoryGame({ shop: false }); improved.upgrade(improved.at(5, 2).id); improved.upgrade(improved.at(3, 2).id); improved.acceptContract(1); run(improved, 60); assert.equal(improved.contract.status, 'ready');
