...
test('cat shop branding, primary sushi entry and generated share image retain all earlier modes', async () => {
  const root = new URL('../', import.meta.url);
  const html = await readFile(new URL('index.html', root), 'utf8');
  ...
  assert.ok(html.indexOf('id="menu-sushi"') < html.indexOf('class="classic-modes"'));
  for (const id of ['menu-sort', 'menu-kitchen', 'menu-play', 'menu-craft', 'menu-packing']) assert.ok(html.includes(`id="${id}"`));
