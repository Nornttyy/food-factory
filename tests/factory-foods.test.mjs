...
test('new food orders cover the expanded menu with bounded repeat demands', () => {
  ...
  for (const bad of [0, 3, '2', null]) { const saved = JSON.parse(before); saved.orderCatalog = bad; assert.equal(game.restore(JSON.stringify(saved)), false); assert.equal(game.serialize(), before); }
});
