...
export function contractFor(tier, round, slot) {
  const [title, tag, wants, duration, reward] = CONTRACTS[tier][slot];
  const growth = Math.min(10, Math.floor(round / 3));
  return { tier, round, slot, title, tag, wants: Object.fromEntries(Object.entries(wants).map(([item, n]) => [item, Math.min(80, Math.ceil(n * (1 + growth * .12)))])), duration, reward: reward + growth * 60, points: slot + 1 };
}
export function contractComplete(contract) {
  const def = contractFor(contract.tier, contract.round, contract.slot);
...
export function validCareer(career, state) {
  ...
  const def = contractFor(c.tier, c.round, c.slot);
  ...
  const complete = contractComplete(c);
