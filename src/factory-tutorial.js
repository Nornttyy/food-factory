import { AutomaticFactoryGame as FactoryGame } from './factory-automation.js?v=0.21.0';
import { canLink } from './factory-links.js?v=0.21.0';
...
export const LESSONS = [
  ...
  { title: '看第一份面包自动出货', copy: '面包进入出货口后自动售卖，不用拖动。', cell: { x: 8, y: 2 } },
  { title: '让烤箱更快一点', copy: '点发光的烤箱，再点「升级」。', cell: { x: 5, y: 2 }, target: 'upgrade-building' },
  { title: '领走第一笔订单奖励', copy: '自动售出 4 份后，在右上角「订单」领取奖励。', target: 'orders-toggle' },
  { title: '学会啦，正式开工！', copy: '接产线 → 自动出货 → 解锁新美食。也能把出货口切为自动入库，完成合作订单。' },
];
