...
export class FactoryCamera {
  ...
  resize(width, height, area, ui = {}) {
    if (area.join(',') !== this.areaKey) { this.areaKey = area.join(','); this.fit(area); }
  ...
  compose() {
    ...
    const base = Math.min(w / (area[0] * CELL), h / (area[1] * CELL));
    const scale = Math.max(.08, base * this.zoom);
    this.transform = { scale, x: i.left + w / 2 - this.x * scale, y: i.top + h / 2 - this.y * scale };
    return this.transform;
  }
  fit(area) { this.x = area[0] * CELL / 2; this.y = area[1] * CELL / 2; this.zoom = 1; this.compose(); }
  zoomAt(factor, px, py) {
    ...
    this.zoom = clamp(this.zoom * factor, .65, 2.6);
  ...
  bound() { this.x = clamp(this.x, 0, 14 * CELL); this.y = clamp(this.y, 0, 8 * CELL); }
  ensureCell(x, y) {
    if (!this.view) return;
