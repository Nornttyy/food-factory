import { WIDTH, HEIGHT, BUILDINGS, ITEMS, DIRS } from './factory-core.js?v=0.21.0';
import { CELL, FactoryCamera, jellyPose, foodPose, presentationTime } from './factory-feel.js?v=0.21.0';
import { conveyorPorts, connectedPorts } from './factory-links.js?v=0.21.0';
import { worldArea, yardLayout, drawYardGround } from './factory-yard.js?v=0.21.0';
import { PACK_RECIPES, ingredientsReady } from './factory-packing.js?v=0.21.0';
...
export class FactoryAssets {
  ...
  async load(base = './assets/generated/factory/cream-v1/', onProgress = () => {}) {
    ...
    onProgress(0, 1);
    const loading = (async () => {
      const response = await fetch(base + 'manifest.json?v=0.21.0', { signal: controller.signal });
...
export class FactoryRenderer {
  ...
  draw(game, ui, timestamp) {
    ...
    if (ui.hover && game.inside(ui.hover.x, ui.hover.y)) {
      ...
      if (ui.tool === 'handmade') this.assets.draw(ctx, 'donut_strawberry', x * CELL + 17, y * CELL + 16, 38, 38, .75);
    }
